...
API.init = function init(callback, allContracts, path, provider, configName, lookbackIn) {
  ...
  this.web3.version.getNetwork((error, version) => {
    ...
    // path
    if (path) {
      this.config.contractRyxEx = path + this.config.contractRyxEx;
      this.config.contractToken = path + this.config.contractToken;
    }

    // contracts
    this.contractRyxEx = undefined;
    this.contractRyxExAddrs = [this.config.contractRyxExAddrs[0].addr];
    if (allContracts) {
      this.contractRyxExAddrs = this.config.contractRyxExAddrs.map(x => x.addr);
    }
    ...
    async.series(
      [
        (callbackSeries) => {
          this.utility.loadContract(
            ...
            this.contractRyxExAddrs[0],
            (err, contract) => {
              this.contractRyxEx = contract;
      ...
      () => {
        callback(null, {
          contractRyxEx: this.contractRyxEx,
...
API.logs = function logs(callback, lookbackIn) {
  this.utility.blockNumber(this.web3, (err, blockNumber) => {
    ...
    async.mapSeries(
      this.contractRyxExAddrs,
      (contractRyxExAddr, callbackMap) => {
        const blocks = Object.values(this.eventsCache)
          .filter(x => x.address === contractRyxExAddr)
          .map(x => x.blockNumber);
...
API.getRyxExBalance = function getRyxExBalance(addr, callback) {
  if (addr.length === 42) {
...
API.getRyxExTokenBalances = function getRyxExTokenBalances(addr, callback) {
  if (addr.length === 42) {
    async.reduce(
      this.config.tokens,
      {},
...
API.getUSDBalance = function getUSDBalance(addr, tokenPrices, callback) {
  async.parallel(
    [
      ...
      (callbackParallel) => {
        API.getRyxExTokenBalances(addr, callbackParallel);
      },
      (callbackParallel) => {
    ...
    (err, results) => {
      const balances = { Wallet: results[0], RyxEx: results[1] };
...
API.addOrderFromEvent = function addOrderFromEvent(event, callback) {
  if (event.event === 'Order' && event.address === this.contractRyxExAddrs[0]) {
...
API.getTrades = function getTrades(callback) {
  ...
  events.forEach((event) => {
    if (event.event === 'Trade' && this.contractRyxExAddrs.indexOf(event.address) >= 0) {
...
API.getFees = function getFees(callback) {
  const fees = [];
  const feeTake = new BigNumber(0.000);
...
API.getDepositsWithdrawals = function getDepositsWithdrawals(callback) {
  ...
  const events = Object.values(this.eventsCache);
  events.forEach((event) => {
    if (event.event === 'Deposit' && this.contractRyxExAddrs.indexOf(event.address >= 0)) {
      ...
      event.event === 'Withdraw' && this.contractRyxExAddrs.indexOf(event.address) >= 0
    ) {
      if (event.args.amount.toNumber() > 0) {
        const token = API.getToken(event.args.token);
        depositsWithdrawals.push({
      ...
      if (trade.token && trade.base && trade.base.name === 'ETH') {
        ...
        if (!tickers[pair]) {
          ...
          ordersFiltered = ordersFiltered.filter(
            order => order.order.contractAddr === this.config.contractRyxExAddrs[0].addr);
      ...
      if (balance.lt(new BigNumber(amountGive))) {
        ...
        this.utility.sign(this.web3, addr, hash, pk, (errSign, sig) => {
          if (errSign) {
            ...
            const order = {
              contractAddr: this.contractRyxExAddrs[0],
              tokenGet,
              amountGet,
              tokenGive,
              amountGive,
