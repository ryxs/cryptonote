...
RyxEx.prototype.ejs = function ejs(url, element, data) {
  if ($(`#${element}`).length) {
...
RyxEx.prototype.dialogInfo = function dialogInfo(message) {
  console.log(message);
...
RyxEx.prototype.dialogError = function dialogError(message) {
  console.log(message);
...
RyxEx.prototype.alertSuccess = function alertSuccess(message) {
  console.log(message);
...
RyxEx.prototype.txError = function txError(err) {
  ...
  utility.getBalance(this.web3, this.addrs[this.selectedAccount], (errBalance, resultBalance) => {
    ...
    if (this.connection.connection === 'RPC' && this.addrKinds[this.selectedAccount] !== 'MetaMask') {
      if (this.pks[this.selectedAccount]) {
        this.dialogError('You are using an RYXEX account that has a private key attached, but you\'re connected to MetaMask. You should disable MetaMask from Chrome\'s Window -> Extensions menu (don\'t worry, this won\'t lose your MetaMask data), then refresh RyxEx.');
        ga('send', {
          hitType: 'event',
          eventCategory: 'Error',
      ...
      this.dialogError('You are using an RYXEX account that doesn\'t have a private key attached. Perhaps you created the account using MetaMask, in which case you should make sure MetaMask is enabled and logged in to this account, then refresh RyxEx. Or, if you have the private key, you can choose "Import account" from the accounts dropdown (upper right) to re-import the account with its private key.');
      ga('send', {
        hitType: 'event',
        eventCategory: 'Error',
        eventAction: 'Ethereum - transaction error',
      ...
      this.dialogError('You are using an RyxEx account that has an invalid private key.');
      ga('send', {
        hitType: 'event',
        eventCategory: 'Error',
        eventAction: 'Ethereum - transaction error',
      ...
      this.dialogError(
        `Your wallet's ETH balance (${balance} ETH) is not enough to cover the gas cost (Ethereum network fee). RYXEX sends ${this.minGas} ETH with each transaction. This is an overestimate and the excess will get refunded to you. It's a good idea to send more than ${this.minGas} so you can pay for not only this transaction, but also future transactions you do on RyxEx. The gas has to come directly from your Wallet (RyxEx has no physical way of paying gas from your deposited ETH).`);
      ga('send', {
        hitType: 'event',
        eventCategory: 'Error',
        eventAction: 'Ethereum - transaction error',
...
RyxEx.prototype.alertTxResult = function alertTxResult(err, txsIn) {
...
RyxEx.prototype.enableTooltipsAndPopovers = function enableTooltipsAndPopovers() {
  $('[data-toggle="popover"]').popover({ trigger: 'hover' });
  $('[data-toggle="tooltip"]').tooltip();
};
RyxEx.prototype.createAccount = function createAccount() {
...
RyxEx.prototype.deleteAccount = function deleteAccount() {
  if (this.pks[this.selectedAccount]) {
...
RyxEx.prototype.selectAccount = function selectAccount(i) {
...
RyxEx.prototype.addAccount = function addAccount(newAddr, newPk) {
...
RyxEx.prototype.showPrivateKey = function showPrivateKey() {
  const addr = this.addrs[this.selectedAccount];
  const pk = this.pks[this.selectedAccount];
  if (!pk) {
    this.dialogError(
...
RyxEx.prototype.addressLink = function addressLink(address) {
  return `https://${this.config.ethTestnet ? `${this.config.ethTestnet}.` : ''}etherscan.io/address/${address}`;
};
RyxEx.prototype.contractAddr = function contractAddr(addr) {
  this.config.contractRyxExAddr = addr;
  this.displayConnectionDescription(() => {});
  this.loading(() => {});
  this.refresh(() => {}, true, true);
};
RyxEx.prototype.displayAccounts = function displayAccounts(callback) {
...
RyxEx.prototype.displayLanguages = function displayLanguages(callback) {
...
RyxEx.prototype.selectLanguage = function selectLanguage(newLanguage) {
...
RyxEx.prototype.loadEvents = function loadEvents(callback) {
  let lastBlock = 0;
  Object.keys(this.eventsCache).forEach((id) => {
    const event = this.eventsCache[id];
    if (event.blockNumber > lastBlock && event.address === this.config.contractRyxExAddr) {
...
RyxEx.prototype.displayMyTransactions =
function displayMyTransactions(ordersIn, blockNumber, callback) {
  ...
  orders = orders.filter(order => order.order.contractAddr === this.config.contractRyxExAddr);
  ...
  events.forEach((event) => {
    try {
      if (
        event.event === 'Trade' &&
        event.address === this.config.contractRyxExAddr &&
...
RyxEx.prototype.displayVolumes = function displayVolumes(
  ...
  Object.keys(pairVolumes).forEach((pair) => {
    ...
    ordersFiltered = ordersFiltered.filter(
      order => order.order.contractAddr === this.config.contractRyxExAddr);
    // final order filtering and sorting
...
RyxEx.prototype.displayTradesAndChart = function displayTradesAndChart(callback) {
  // get the trade list
  const events = Object.values(this.eventsCache);
  const trades = [];
  events.forEach((event) => {
    if (event.event === 'Trade' && event.address === this.config.contractRyxExAddr) {
      if (event.args.amountGive.toNumber() > 0 && event.args.amountGet.toNumber() > 0) {
        // don't show trades involving 0 amounts
        let trade;
        if (
...
RyxEx.prototype.candlestickChart =
...
RyxEx.prototype.getOrdersByPair = function getOrdersByPair(tokenA, tokenB, callback) {
...
RyxEx.prototype.getReturnTicker = function getTopOrders(callback) {
...
RyxEx.prototype.getTopOrders = function getTopOrders(callback) {
...
RyxEx.prototype.displayOrderbook = function displayOrderbook(ordersIn, blockNumber, callback) {
  // only look at orders for the selected token and base
  let orders = ordersIn.filter(
    x =>
      (x.order.tokenGet.toLowerCase() === this.selectedToken.addr.toLowerCase() &&
...
RyxEx.prototype.displayTokensAndBases = function displayTokensAndBases(callback) {
...
RyxEx.prototype.displayAllBalances = function displayAllBalances(callback) {
...
RyxEx.prototype.transfer = function transfer(addr, inputAmount, toAddr) {
  ...
  } else if (toAddr.toLowerCase() === this.config.contractRyxExAddr.toLowerCase()) {
    this.dialogError('If you send funds directly to the RYXEX smart contract, they will be lost. You need to use the Deposit tab to deposit.');
    ga('send', {
      hitType: 'event',
      eventCategory: 'Error',
      eventAction: 'Transfer - invalid address',
...
RyxEx.prototype.deposit = function deposit(addr, inputAmount) {
  let amount = new BigNumber(Number(utility.ethToWei(inputAmount, this.getDivisor(addr))));
  const token = this.getToken(addr);
  if (amount.lte(0)) {
  ...
  if (addr.slice(0, 39) === '0x0000000000000000000000000000000000000') {
    ...
    utility.call(
      ...
      token.addr,
      'allowance',
      [this.addrs[this.selectedAccount], this.config.contractRyxExAddr],
                  ...
                  (callbackSeries) => {
                    if (resultAllowance.eq(0)) {
                      utility.send(
                        ...
                        addr,
                        'approve',
                        [this.config.contractRyxExAddr, amount,
...
RyxEx.prototype.withdraw = function withdraw(addr, amountIn) {
...
RyxEx.prototype.order = function order(direction, amount, price, expires, refresh) {
...
RyxEx.prototype.publishOrder = function publishOrder(
      ...
      if (balance.lt(new BigNumber(amountGive))) {
        ...
        hash, this.pks[this.selectedAccount], (errSign, sig) => {
          if (errSign) {
            ...
            const order = {
              contractAddr: this.config.contractRyxExAddr,
              tokenGet,
              amountGet,
              tokenGive,
              amountGive,
...
RyxEx.prototype.cancelOrder = function cancelOrder(orderIn) {
...
RyxEx.prototype.trade = function trade(kind, order, inputAmount) {
  if (this.addrs[this.selectedAccount].slice(0, 39) === '0x0000000000000000000000000000000000000') {
    this.dialogError(
      "You haven't selected an account. Make sure you have an account selected from the Accounts dropdown in the upper right.");
    ga('send', {
...
RyxEx.prototype.addPending = function addPending(err, txsIn) {
  const txs = Array.isArray(txsIn) ? txsIn : [txsIn];
  txs.forEach((tx) => {
...
RyxEx.prototype.updateUrl = function updateUrl() {
...
RyxEx.prototype.getDivisor = function getDivisor(tokenOrAddress) {
...
RyxEx.prototype.getToken = function getToken(addrOrToken, name, decimals) {
  let result;
...
RyxEx.prototype.loadToken = function loadToken(addr, callback) {
...
RyxEx.prototype.selectToken = function selectToken(addrOrToken, name, decimals) {
...
RyxEx.prototype.selectBase = function selectBase(addrOrToken, name, decimals) {
...
RyxEx.prototype.selectTokenAndBase = function selectTokenAndBase(tokenAddr, baseAddr) {
...
RyxEx.prototype.setGasPrice = function setGasPrice(gasPrice) {
  if (gasPrice) {
    this.config.ethGasPrice = Number(gasPrice) * 1000000000;
    this.minGas = (this.config.ethGasPrice * this.config.gasDeposit) / (10 ** 18);
  }
};
RyxEx.prototype.displayBuySell = function displayBuySell(callback) {
...
RyxEx.prototype.displayTokenGuidesDropdown = function displayTokenGuidesDropdown() {
  const tokens = this.config.tokens.map(x => x);
  tokens.sort((a, b) => (a.name > b.name ? 1 : -1));
...
RyxEx.prototype.displayHelpDropdown = function displayHelpDropdown() {
  this.ejs(`${this.config.homeURL}/templates/helpDropdown.ejs`, 'helpDropdown', {});
};
RyxEx.prototype.displayHelp = function displayHelp(name) {
  $('#helpBody').html('');
...
RyxEx.prototype.displayScreencast = function displayScreencast(name) {
  $('#screencastBody').html('');
...
RyxEx.prototype.displayConnectionDescription = function displayConnectionDescription() {
  this.ejs(`${this.config.homeURL}/templates/connectionDescription.ejs`, 'connection', {
    connection: this.connection,
    contracts: this.config.contractRyxExAddrs,
    contractAddr: this.config.contractRyxExAddr,
    contractLink: `https://${this.config.ethTestnet ? `${this.config.ethTestnet}.` : ''}etherscan.io/address/${this.config.contractRyxExAddr}`,
  });
};
RyxEx.prototype.displayTokenGuide = function displayTokenGuide(name) {
...
RyxEx.prototype.checkContractUpgrade = function checkContractUpgrade() {
  if (
    (!this.selectedContract || this.selectedContract !== this.config.contractRyxExAddr) &&
    (this.addrs.length > 1 ||
      (this.addrs.length === 1 && this.addrs[0].slice(0, 39) !== '0x0000000000000000000000000000000000000'))
  ) {
    this.dialogInfo(
      '<p>RyxEx has a new smart contract. It is now selected.</p><p>Please use the "Smart Contract" menu to select the old one and withdraw from it.</p><p><a href="javascript:;" class="btn btn-default" onclick="alertify.closeAll(); bundle.RyxEx.displayHelp(\'smartContract\')">Smart contract changelog</a></p>');
  }
};
RyxEx.prototype.resetCaches = function resetCaches() {
...
RyxEx.prototype.loading = function loading(callback) {
  [
    'deposit',
    'withdraw',
    'buy',
...
RyxEx.prototype.refresh = function refresh(callback, forceEventRead, initMarket, token, base) {
  this.q.push((done) => {
    if (token && base) {
    ...
    this.selectedContract = this.config.contractRyxExAddr;
...
RyxEx.prototype.refreshLoop = function refreshLoop() {
  const self = this;
  function loop() {
    self.refresh(() => {
...
RyxEx.prototype.initDisplays = function initDisplays(callback) {
  this.loading(() => {});
...
RyxEx.prototype.loadWeb3 = function loadWeb3(callback) {
  this.config = config;
  // web3
...
RyxEx.prototype.initContracts = function initContracts(callback) {
  this.web3.version.getNetwork((error, version) => {
    ...
    // connection
    this.config.contractRyxExAddr = this.config.contractRyxExAddrs[0].addr;
    // get accounts
    ...
    utility.loadContract(
      this.web3,
      this.config.contractRyxEx,
      this.config.contractRyxExAddr,
      (err, contractRyxEx) => {
        this.contractRyxEx = contractRyxEx;
        utility.loadContract(
          this.web3,
...
RyxEx.prototype.startRyxEx = function startRyxEx() {
...
const ryxEx = new RyxEx();

module.exports = { RyxEx: ryxEx, utility };
