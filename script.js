...
$(() => {
  $('body').on('click', '#accountSubmit', (e) => {
    ...
    bundle.RyxEx.addAccount($('#accountAddr').val(), $('#accountPk').val());
  });
});
...
$('#buyCrossModal').on('show.bs.modal', (e) => {
  ...
  $('#buyCrossFee').val((amount * price * 0.000).toFixed(3));
  $('#buyCrossBaseAmount').change(() => {
    $('#buyCrossAmount').val((Number($('#buyCrossBaseAmount').val()) / price).toFixed(3));
    $('#buyCrossFee').val(($('#buyCrossBaseAmount').val() * 0.000).toFixed(3));
  });
...
$(() => {
  $('body').on('click', '#buyCrossSubmit', (e) => {
    ...
    bundle.RyxEx.trade('buy', JSON.parse($('#buyCrossOrder').val()), $('#buyCrossAmount').val());
  });
});
$(() => {
...
$('#gasPriceModal').on('show.bs.modal', () => {
  $('#gasPrice').val(bundle.RyxEx.config.ethGasPrice / 1000000000);
});
$(() => {
  $('body').on('click', '#gasPriceSubmit', (e) => {
    e.preventDefault();
    $('#gasPriceModal').modal('hide');
    bundle.RyxEx.setGasPrice(
      $('#gasPrice').val());
  });
});
function depositClick(addr) { // eslint-disable-line no-unused-vars
  bundle.RyxEx.deposit(addr, $(`#depositAmount${addr}`).val());
}
function withdrawClick(addr) { // eslint-disable-line no-unused-vars
  bundle.RyxEx.withdraw(addr, $(`#withdrawAmount${addr}`).val());
}
function transferClick(addr) { // eslint-disable-line no-unused-vars
  bundle.RyxEx.transfer(addr, $(`#transferAmount${addr}`).val(), $(`#transferTo${addr}`).val());
}
$(() => {});
