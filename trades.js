...
const addressRyxEx = '0x56a20f5c2ad2d88d635783780a7a1ad0e50df492';
const addressToken = '0x00e5894dB35FbaEe76732aa5eed712BA67E90662';
...
function TradeUtil() {
  ...
  self.getContract = function getContract(callback) {
    self.get(`https://api.etherscan.io/api?module=contract&action=getabi&address=${addressRyxEx}`, (err, data) => {
      if (err) throw new Error(err);
      const abi = JSON.parse(data.result);
      self.contractRyxEx = web3.eth.contract(abi);
      self.contractRyxEx = self.contractRyxEx.at(addressRyxEx);
      callback(null, self.contractRyxEx);
    });
  };
  ...
  self.getLog = function getLog(fromBlock, toBlock, callback) {
    function decodeEvent(item) {
      const eventAbis = self.contractRyxEx.abi.filter(eventAbi => (
          eventAbi.type === 'event' &&
          item.topics[0] ===
            `0x${
              sha3(
      ...
      if (eventAbis.length > 0) {
        ...
        const event = new SolidityEvent(web3, eventAbi, addressRyxEx);
    ...
    const url =
      `https://api.etherscan.io/api?module=logs&action=getLogs&address=${addressRyxEx}&fromBlock=${fromBlock}&toBlock=${toBlock}`;
